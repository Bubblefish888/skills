...
| 情况 | 处理 |
|------|------|
| 只是说"帮帮我"没具体情境 | "先说说现在什么情况？你们认识多久了，目前到什么阶段了？" |
| 聊天记录里关键信息模糊 | 追问："这几句话是在什么时候发的？之前你们在聊什么话题？" |
| 用户情绪焦虑/被拒绝 | 先稳情绪再分析，不说"放轻松"这种空话，给出具体的下一步行动建议 |

### 防耗尽机制

`/simp daily` 和 `/simp progress` 执行前预检：
- 读 `events.jsonl` 最近 7 天记录
- 命中高频询问/焦虑词密度高/评分持续下降 → 劝用户暂停，不要推更多追求行动

### 用户被拒/冷落

1. 不做空安慰，不给"下一个更好"这种话
2. 分析具体原因（时机不对/对方状态/表达方式），给 1-2 个可操作的建议
3. 如果明显已无机会，诚实告知，建议止损
